 ...
 * lifecycle over HTTP: login -> create -> list -> patch status -> trigger
 * recon -> resolve break. @ServiceConnection wires the DataSource to the
 * container automatically, no manual @DynamicPropertySource needed.
 * ============================================================================
 */
@Testcontainers
...
class TradeLifecycleIT {
    ...
    // The JDK's HttpURLConnection, which SimpleClientHttpRequestFactory wraps,
    // rejects PATCH outright ("Invalid HTTP method: PATCH"). JdkClientHttpRequestFactory
    // uses java.net.http.HttpClient instead, which supports it.
    ...
    @Order(6)
    void resolveBreak() {
        // Test data seeded by Liquibase guarantees at least one OPEN break with id 1.
        breakId = 1L;
        var body = """
