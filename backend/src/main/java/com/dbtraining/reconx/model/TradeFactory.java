...
public final class TradeFactory {
    ...
    /**
     * Dispatch on the asset-class discriminator and build the matching
     * {@link TradeType} from a loosely-typed parameter map.
     *
     * @param assetClass one of {@code EQUITY}/{@code FX}/{@code BOND}/{@code DERIVATIVE}, case-insensitive
     * @param p          field values keyed by name, as parsed from the inbound payload
     * @return a fully-built, validated {@link TradeType} — never {@code null}
     * @throws IllegalArgumentException if {@code assetClass} does not match a known {@link TradeType.AssetClass}
     * @throws NullPointerException     if a required key is missing from {@code p}
     * @throws ClassCastException       if a value in {@code p} is not the expected type
     */
